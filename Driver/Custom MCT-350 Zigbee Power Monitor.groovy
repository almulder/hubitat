/**
*  Custom MCT-350 Zigbee Power Monitor
*
...
*
*
*	Version 1.0 - Initial Driver
*	Version 1.2 - Updated to Keep powerMains on screen
*
*/
...
metadata {
    definition (name: "Custom MCT-350 Zigbee Power Monitor", namespace: "almulder", author: "Albert Mulder") {
        capability "Battery"
...
def handleZoneStatus(String status) {
    ...
    if (powerState) {
        ...
        sendEvent(name: "powerMains", value: powerState, displayed: true, isStateChange: true)
    }

    if (tamperState) {
        log.info "Tamper ${tamperState}"
        sendEvent(name: "tamper", value: tamperState, displayed: true, isStateChange: true)
    }
}
...
def refresh() {
    ...
    sendEvent(name: "powerMains", value: device.currentValue("powerMains") ?: "off", displayed: true)
...
def installed() {
    ...
    sendEvent(name: "powerMains", value: "off", displayed: true)
    sendEvent(name: "tamper", value: "detected", displayed: true)
    sendEvent(name: "battery", value: 100, unit: "%", displayed: true)
    sendEvent(name: "temperature", value: 25.0, unit: "C", displayed: true)
}
...
def updated() {
    ...
    sendEvent(name: "powerMains", value: device.currentValue("powerMains") ?: "off", displayed: true)
    sendEvent(name: "tamper", value: device.currentValue("tamper") ?: "detected", displayed: true)
    sendEvent(name: "battery", value: device.currentValue("battery") ?: 100, unit: "%", displayed: true)
    sendEvent(name: "temperature", value: device.currentValue("temperature") ?: 25.0, unit: "C", displayed: true)
}
